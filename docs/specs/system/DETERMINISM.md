...
## 2.1 SSOT Config（実装基準）

- `diycad_common::determinism::DeterminismConfig` を決定性パラメータの SSOT とする。
  - `seed: u64`
  - `epsilon: f64`
  - `rounding_decimals: u8`
- seed 由来 RNG・丸め・ε比較はこの設定と整合して運用する。
...
- 決定性を破る変更（同入力で異なる乱数列/丸め結果/比較結果を生む変更）は **NG**。
- 例外が必要な場合は、設計レビューと `DECISION_LOG.md` への記録を必須とする。
