...
use golden_harness::{
    ...
    compare_reason_codes, hash_bytes, write_failure_artifacts, ActualData, CompareKind, DatasetMeta,
    ExpectedEntry, InputRef,
};
...
fn golden_harness_smoke_json_and_reasoncodes_and_bytes() {
    ...
    compare_json_struct(&root, &meta, &expected_json, json!({"b":{"x":2}, "a": 1.000049})).unwrap();

    let codes = canonical_reason_codes(&json!({"warnings":[{"code":"B"},{"code":"A"},{"code":"A"}]}));
