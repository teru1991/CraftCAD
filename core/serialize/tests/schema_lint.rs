...
fn schema_ids_are_unique_and_validated() {
    ...
    let man_raw = std::fs::read_to_string("schemas/manifest.schema.json").expect("read manifest schema");
    ...
    let sample = std::fs::read_to_string("../../tests/fixtures/sample_project_v1.json").expect("fixture");
