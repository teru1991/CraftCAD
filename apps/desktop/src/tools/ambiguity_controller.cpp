...
void AmbiguityController::render(QPainter& p, const Camera& camera) const {
    ...
    for (int i=0;i<candidates_.size();++i){ auto o=candidates_[i].toObject(); QPointF s=camera.worldToScreen(WVec2{o.value("x").toDouble(),o.value("y").toDouble()}); p.setPen(QPen(i==index_?QColor(255,255,0):QColor(255,120,120),1)); p.drawEllipse(s, i==index_?6:4, i==index_?6:4); }
}
