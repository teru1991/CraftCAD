...
void RectTool::onPointerMove(const QPointF& s){ if(!p0_) return; WVec2 w=camera_->screenToWorld(s); snap_=computeSnap(*store_,w,p0_); p1_=snap_.best?snap_.best->point:w; if(lockH) p1_->y=p0_->y; if(lockV) p1_->x=p0_->x; }
void RectTool::onPointerUp(const QPointF&){ if(!p0_||!p1_) return; if(lockH&&lockV){ QMessageBox::warning(nullptr,"Rect","DRAW_CONSTRAINT_CONFLICT"); p0_.reset(); p1_.reset(); return; }
...
void RectTool::onKeyPress(QKeyEvent* e){ if(e->key()==Qt::Key_H) lockH=!lockH; if(e->key()==Qt::Key_V) lockV=!lockV; if(e->key()==Qt::Key_Escape){ p0_.reset(); p1_.reset(); numeric_.clear(); } numeric_.handleKey(e->key(),e->text()); }
