...
pub fn parse_market_meta_snapshot_from_str(
    ...
    let payload: ApiResp<Vec<SymbolRow>> =
        serde_json::from_str(body).map_err(|e| MarketMetaAdapterError::Mapping(e.to_string()))?;

    let mut markets = Vec::with_capacity(payload.data.len());
    for r in payload.data {
        let id = MarketMetaId::new(Exchange::Gmocoin, MarketType::Spot, r.symbol);
        let mut meta = MarketMeta::new(
            id,
            parse_dec(&r.tick_size, "tick_size")?,
            parse_dec(&r.size_step, "size_step")?,
        );
        meta.min_qty = Some(parse_dec(&r.min_order_size, "min_order_size")?);
        meta.max_qty = Some(parse_dec(&r.max_order_size, "max_order_size")?);
        meta.validate_meta()
